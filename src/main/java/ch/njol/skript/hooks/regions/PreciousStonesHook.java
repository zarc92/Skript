...
public class PreciousStonesHook extends RegionsPlugin<PreciousStones> {
	...
	public Collection<? extends Region> getRegionsAt_i(final Location l) {
		Set<PreciousStonesRegion> collect = PreciousStones.API().getFieldsProtectingArea(FieldFlag.ALL, l).stream().map(PreciousStonesRegion::new).collect(Collectors.toSet());
		assert collect != null;
		return collect;
	}
	...
	public final class PreciousStonesRegion extends Region {
		...
		public Collection<OfflinePlayer> getMembers() {
			...
			Set<OfflinePlayer> collect = field.getAllAllowed().stream().map(Bukkit::getOfflinePlayer).collect(Collectors.toSet());
			assert collect != null;
			return collect;
		}
		...
		public Collection<OfflinePlayer> getOwners() {
			@SuppressWarnings("deprecation")
			Set<OfflinePlayer> collect = Stream.of(Bukkit.getOfflinePlayer(field.getOwner())).collect(Collectors.toSet());
			assert collect != null;
			return collect;
		}
