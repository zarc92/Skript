...
public class Direction implements YggdrasilRobustSerializable {
	...
	private Vector getDirection(final double p, final double y) {
		...
		return new Vector(Math.cos(y + yawOrY) * lxz, Math.sin(p + pitchOrX) * Math.cos(yawOrY) * lengthOrZ,
				Math.sin(y + yawOrY) * lxz);
	}
	...
	private static void toString(final StringBuilder b, final double d, final Message direction,
			final Message oppositeDirection, final boolean prependAnd) {
		if (d == 0)
			return;
		if (prependAnd)
...
//		return "" + relative + ":" + (relative ? pitch + "," + yaw + "," + length : mod[0] + "," + mod[1] + "," + mod[2]);
	...
	public static Direction deserialize(final String s) {
		...
		if (relative) {
			...
			if (split2.length != 3)
				return null;
			try {
				return new Direction(Double.parseDouble(split2[0]), Double.parseDouble(split2[1]),
			...
			try {
				return new Direction(
						new double[] {Double.parseDouble(split2[0]), Double.parseDouble(split2[1]), Double.parseDouble(split2[2])});
			} catch (final NumberFormatException e) {
				return null;
			}
		}
	...
	public static Expression<Location> combine(final Expression<? extends Direction> dirs,
			final Expression<? extends Location> locs) {
