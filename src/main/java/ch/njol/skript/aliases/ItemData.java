...
public class ItemData implements Cloneable, YggdrasilSerializable {
	...
	public ItemData(final int typeid, final short dMin, final short dMax) {
		...
		if (dMin == -1 && dMax != -1 || dMin != -1 && dMax == -1)
			throw new IllegalArgumentException(
					"dataMin (" + dMin + ") and dataMax (" + dMax + ") must either both be -1 or positive");
		if (dMin > dMax)
	 ...
	 * 
	 * @param item
	 * @return Whether the given item is of this type. If <tt>item</tt> is <tt>null</tt> this returns
	 *         <tt>getId() == 0</tt>.
	 */
	...
	/**
	 * Returns
	 * <code>Aliases.{@link Aliases#getMaterialName(int, short, short, boolean) getMaterialName}(typeid, dataMin, dataMax, false)</code>
	 */
	@Override
	 ...
	 * intersection of the two data ranges, and the type id will be the one
	 * set if any.
	 * 
	 * @param other
	 * @return A new ItemData which is the intersection of the given types, or null if the intersection of the data
	 *         ranges is empty or both datas have an id != -1 which are not the
	 *         same.
	 */
	@Nullable
	public ItemData intersection(final ItemData other) {
		...
		return new ItemData(typeid == -1 ? other.typeid : typeid, (short) Math.max(dataMin, other.dataMin),
