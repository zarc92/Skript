...
public class CondCompare extends Condition {
	...
	private boolean init(String expr) {
		...
		final Class<?> f = first.getReturnType(),
				s = third == null ? second.getReturnType() : Utils.getSuperType(second.getReturnType(), third.getReturnType());
		 ...
		 * if(!split[split.length - 1].equalsIgnoreCase("player") &&
		 * EntityData.parseWithoutIndefiniteArticle(split[split.length - 1]) != null){
	 ...
	 * a or b !# x === a !# x || b !# x
	 * a !# x and y === a !# x || a !# y // e.g. "player doesn't have 2 emeralds and 5 gold ingots" ==
	 * "NOT(player has 2 emeralds and 5 gold ingots)" ==
	 * "player doesn't have 2 emeralds OR player doesn't have 5 gold ingots"
	 * a # neither x nor y === a !# x && a !# y // nor = or // e.g. "player has neither 2 emeralds nor 5 gold ingots" ==
	 * "player doesn't have 2 emeralds AND player doesn't have 5 gold ingots"
	 * a # neither x nor y === a !# x && a !# y // nor = or // e.g. "player is neither the attacker nor the victim" ==
	 * "player is not the attacker AND player is not the victim"
	 * a !# x or y === a !# x && a !# y // e.g. "player doesn't have 2 emeralds or 5 gold ingots" ==
	 * "NOT(player has 2 emeralds or 5 gold ingots)" ==
	 * "player doesn't have 2 emeralds AND player doesn't have 5 gold ingots"
	 * a and b !# x and y === a !# x and y && b !# x and y === (a !# x || a !# y) && (b !# x || b !# y)
	 * a and b !# x or y === a !# x or y && b !# x or y
