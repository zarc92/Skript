...
//		return getInfo((Class<? extends EntityData<?>>) d.getClass()).codeName + ":" + d.serialize();
	...
	static {
		Classes.registerClass(new ClassInfo<>(EntityData.class,
				"entitydata").user("entity ?types?").name("Entity Type").description("The type of an <a href='#entity'>entity</a>, e.g. player, wolf, powered creeper, etc.").usage("<i>Detailed usage will be added eventually</i>").examples("victim is a cow", "spawn a creeper").since("1.3").defaultExpression(new SimpleLiteral<>(
						new SimpleEntityData(Entity.class),
						true)).before("entitytype").parser(new Parser<EntityData>() {
	...
	private final static class EntityDataInfo<T extends EntityData<?>> extends SyntaxElementInfo<T> implements LanguageChangeListener {
		...
		public EntityDataInfo(final Class<T> dataClass, final String codeName, final String[] codeNames,
				final int defaultName, final Class<? extends Entity> entityClass) throws IllegalArgumentException {
	...
	public static <E extends Entity, T extends EntityData<E>> void register(final Class<T> dataClass, final String name,
			final Class<E> entityClass, final String codeName) throws IllegalArgumentException {
		register(dataClass, codeName, entityClass, 0, codeName);
	}

	@SuppressWarnings("unchecked")
	public static <E extends Entity, T extends EntityData<E>> void register(final Class<T> dataClass, final String name,
			final Class<E> entityClass, final int defaultName, final String... codeNames)
	...
	@SuppressWarnings("null")
	@Override
	public final boolean init(final Expression<?>[] exprs, final int matchedPattern, final Kleenean isDelayed,
	 ...
	 */
	@SuppressWarnings({"null", "unchecked"})
	public static <E extends Entity> E[] getAll(final EntityData<?>[] types, final Class<E> type,
			@Nullable World[] worlds) {
